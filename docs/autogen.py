...
if __name__ == '__main__':
    ...
    for page_data in PAGES:
        ...
        if os.path.exists(pwd/path):
            template = read_file(pwd/path)
            assert '{{autogenerated}}' in template, ('Template found for ' + path +
                                                     ' but missing {{autogenerated}}'
                                                     ' tag.')
            mkdown = template.replace('{{autogenerated}}', mkdown)
            print('...inserting autogenerated content into template:', pwd/path)
        else:
            print('...creating new page with autogenerated content:', pwd/path)
        subdir = os.path.dirname(pwd/path)
        if not os.path.exists(pwd/subdir):
            os.makedirs(pwd/subdir)
        with open(pwd/path, 'w') as f:
            f.write(mkdown)
