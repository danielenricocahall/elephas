...
def test_training_classification(
    ...
    # Train Spark model
    spark_model.fit(
        rdd, epochs=epochs, batch_size=batch_size, verbose=0, validation_split=0.1
    )
    ...
    assert isclose(
        evals[0], spark_model.master_network.evaluate(x_test, y_test)[0], abs_tol=0.01
    )
    assert isclose(
        evals[1], spark_model.master_network.evaluate(x_test, y_test)[1], abs_tol=0.01
    )
...
def test_training_regression(
    ...
    assert all(
        np.isclose(x, y, 0.01)
        for x, y in zip(predictions, spark_model.master_network.predict(x_test))
    )

    # assert we get the same evaluation results when calling evaluate on keras model directly
    assert isclose(
        evals, spark_model.master_network.evaluate(x_test, y_test), abs_tol=0.01
    )
...
def test_training_regression_no_metrics(
    ...
    spark_model = SparkModel(
        regression_model, frequency="epoch", mode=Mode.SYNCHRONOUS, port=unused_tcp_port
    )
    ...
    # assert we get the same evaluation results when calling evaluate on keras model directly
    assert isclose(
        spark_model.evaluate(x_test, y_test),
        spark_model.master_network.evaluate(x_test, y_test),
        abs_tol=0.01,
    )
...
def test_multiple_input_model(spark_session, frequency, unused_tcp_port):
    ...
    df = spark_session.read.csv(
        f"{parent_dir}/data/sample_data.csv", header=True, inferSchema=True
    )
    ...
    assembler = VectorAssembler(
        inputCols=["user_id_encoded", "track_id_encoded"], outputCol="features"
    )
    ...
    user_embedding = Embedding(output_dim=n_latent_factors, input_dim=n_users)(
        user_input
    )
    item_embedding = Embedding(output_dim=n_latent_factors, input_dim=n_items)(
        item_input
    )
    ...
    spark_model = AsynchronousSparkModel(
        model, mode=Mode.ASYNCHRONOUS, frequency=frequency, port=unused_tcp_port
    )
    ...
    assert spark_model.evaluate(
        np.array(rdd_test_data.collect()), np.array(rdd_test_targets.collect())
    )
